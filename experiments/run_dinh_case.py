...
def run(
    ...
    expected = [run_dir / "metrics" / "dinh_metrics.csv", run_dir / "data" / "dinh_predictions.csv"]
    if should_skip(run_dir, "dinh_case", expected, resume, force):
        if verbose:
            print("Skipping dinh_case; completed outputs found.")
        return

    case_all = load_case(config)
    progress_message(run_dir, "Dinh case started", verbose, config=config)
    group_col = case_all.group or "rock"
    ...
    for rock in sorted(case_all.frame[group_col].unique().tolist()):
        progress_message(run_dir, "Dinh subgroup started", verbose, rock=rock)
        case = case_all.subset(**{group_col: rock})
        ...
        agent_result = run_agentic_search(
            ...
            metadata={"case": "dinh", "rock": rock, "prior_level": prior_level},
            verbose=verbose,
        )
        result = agent_result.result
        metric_rows.append({"case": "dinh", "rock": rock, "prior_level": prior_level, "method": "baseline", "predictive_status": "baseline", "structural_status": "not_applicable", "formula": "0", "complexity": 0, "physical_violations": 0, **baseline})
        metric_rows.append(
            {
                "case": "dinh",
                "rock": rock,
                ...
                "leave_one_psi_rmse": result.best["leave_one_psi_rmse"],
        ...
        agent_result.candidates.to_csv(run_dir / "formulas" / f"dinh_{rock.replace('.', '')}_candidate_formulas.csv", index=False)
        progress_message(run_dir, "Dinh subgroup finished", verbose, rock=rock, rmse=f"{result.best['rmse']:.6g}")

    metrics = pd.DataFrame(metric_rows)
    predictions = pd.concat(prediction_rows, ignore_index=True)
    predictions.to_csv(run_dir / "data" / "dinh_predictions.csv", index=False)
    write_json(run_dir / "formulas" / "dinh_best_formulas.json", formula_rows)
    write_table_bundle(metrics, run_dir / "metrics" / "dinh_metrics")
    mark_done(run_dir, "dinh_case", {"rows": len(predictions)})
    progress_message(run_dir, "Dinh case finished", verbose, rows=len(predictions))
    if verbose:
        print(f"Dinh case complete: {run_dir}")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="configs/dinh_case.yaml")
